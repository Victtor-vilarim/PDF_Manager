...
class PDFProcessor(Explorer):
    ...
    @staticmethod
    def __verify_file_merge(writer: pypdf.PdfWriter, file_: Path):
    ...
    @staticmethod
    def __verify_file(file_: Path) -> bool:
        if file_.exists() and file_.is_file() and file_.name.endswith('.pdf'):
            return True
    ...
    def merge(self, *files: str | Path) -> None:
        ...
        for file_ in [self.wd / f if not isinstance(f, Path) else f for f in files]:
            self.__verify_file_merge(writer, file_)
    ...
    def listimg(self, file_: str | Path):
        ...
        """
        file_ = self.wd / file_ if not isinstance(file_, Path) else file_
        self.__verify_file(file_)
        reader = pypdf.PdfReader(file_)
        for page in reader.pages:
            for img in page.images:
                yield img

    def extractimg(self, file_: str | Path, n: int = 1) -> None:
        """
    ...
    def extracttxt(self, file_: str | Path, n: int = 1) -> None:
        """
        ...
        with open(self.wd / 'extracted_text.txt', 'w', encoding='utf-8') as txt_file:
            text = reader.pages[int(n) - 1].extract_text()
            txt_file.write(text)

        return

    def extractpage(self, file_: str | Path, n: int = 1) -> None:
        """
        ...
        with open(self.wd / 'extracted_page.pdf', 'wb') as pdf_file:
            writer.add_page(reader.pages[int(n) - 1])
